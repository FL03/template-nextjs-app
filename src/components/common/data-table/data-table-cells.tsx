...
>({
  ...
  const Trigger = ({
    ...
    function formatValueAs(data: number) {
      if (mode.match(/^(count|unique)$/gim)) {
    ...
    const res = summaryFn(mode)(column.id, table.getCoreRowModel().rows);
    ...
    return (
      ...
      >
        <span className={cn('text-end font-mono font-semibold')}>
  ...
  return (
    <DataTableCell ref={ref} className={cn('p-0', className)} {...props}>
      <Select onValueChange={handleValueChange} value={selected}>
        <Trigger className={classNames?.triggerClassName} mode={selected} />
        <SelectContent
