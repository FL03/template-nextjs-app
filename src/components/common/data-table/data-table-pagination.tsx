...
> = ({
  ...
  return (
    <ButtonGroup ref={ref} data-slot='select-page-size' {...props}>
      <Select
        value={getPageSize()}
...
> = ({ ref, className, classNames, hidePageCount, ...props }) => {
  // get the table instance from context
  const { table } = useDataTable();
  // memoize the current page index
  const currentPageIdx = () => table.getState().pagination.pageIndex + 1;
  // render
  return (
    <ButtonGroup
      ref={ref}
