...
export function countOccurrencesInColumn<
  TValue extends RowData,
  TRows extends Array<Row<TValue>>,
>(id: keyof TValue, values: TRows): number {
...
export function countUniqueInColumn<
  TValue extends RowData,
  TRows extends Array<Row<TValue>>,
>(id: keyof TValue, data: TRows): number {
  return data.filter(
    (row, index, self) =>
      self.findIndex((r) => r.original[id] === row.original[id]) === index,
  ).length;
}
...
export function summaryFn<Key extends string>(key: Key): SummaryFnCallback {
