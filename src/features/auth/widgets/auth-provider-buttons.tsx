...
> = ({
  ...
  const checkIsLinked = React.useCallback(async () => {
    // retrieve all identities linked to a user
    const { data, error } = await supabase.auth.getUserIdentities();
    if (error) {
      logger.error(error, 'An error occurred when fetching user identities');
      return false;
    }
    // check if the current provider is already linked
    return data.identities.some((identity) => identity.provider === scope);
  }, [scope, supabase.auth]);
    ...
    const onLink: React.MouseEventHandler<HTMLButtonElement> = () => {
      toast.promise(
        async () => {
          ...
          if (isLinked) {
            logger.warn(
              `The provider ${provider} is already linked to the user's account`,
            );
            return;
          }
  ...
  return (
    <ButtonGroup className={cn('w-full', className)} {...props}>
      ...
      >
        <XPlatformLogo className='size-4' />
      </IconButton>
    </ButtonGroup>
  );
};
