...
export const useStripeProduct: UseProduct.Callback = (
  { productId, onDataChange, onError } = {},
) => {
  ...
  const handleChange = React.useCallback((data: Stripe.Product | null) => (
    _setData((prev) => {
  ...
  const _getPrices = React.useCallback(
    async (
      ...
      await fetchPrices(params).then((data) => (
        _setPrices(data)
      )).catch(
        handleError,
      );
  ...
  const _getProduct = React.useCallback(
    async (id: string) => {
      await fetchProduct({ productId: id }).then(
        handleChange,
      ).catch(
        handleError,
      );
  ...
  const _loader = React.useCallback(async (id: string) => {
    await _getProduct(id).then(async () => {
