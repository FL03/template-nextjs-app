...
export const useShifts: UseShifts.Hook = ({
  ...
  const _fetch = React.useCallback(
    async (assignee?: string): Promise<void> => {
      await getShifts({ assignee }).then(handleDataChange).catch(handleError);
    },
    [handleDataChange, handleError],
  );
  const _deleteShift = React.useCallback(
    async (id: string) => {
  ...
  const _reload = React.useCallback(async (): Promise<void> => {
    ...
    if (!username) {
      logger.error('Unable to reload the shifts for an unauthorized user');
      setIsReloading(false);
